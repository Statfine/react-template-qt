...
const removeEntry = require('../../../helper/ExtensionElementsHelper')
  .removeEntry;
  ...
  var formFieldsEntry = extensionElements(element, bpmnFactory, {
    ...
    createExtensionElement(element, extensionElements, value) {
      ...
      if (!extensionElements) {
        ...
        commands.push(
          cmdHelper.updateProperties(element, { extensionElements }),
        );
      }
      ...
      /** smart 是向 extensionElements 下追加元素，而不是 formData 下， */
      const field = elementHelper.createElement(
        'smart:FormProperty',
        { id: value },
        extensionElements,
        bpmnFactory,
      ...
      if (typeof extensionElements.values !== 'undefined') {
        ...
        commands.push(
          cmdHelper.updateBusinessObject(element, extensionElements, {
            values: [field],
          }),
        );
      }
    ...
    removeExtensionElement(element, extensionElements, value, idx) {
      const fields = formHelper.getFormFields(element);
      const entry = fields[idx];
      const commands = [];

      if (fields.length < 2) {
        commands.push(
          removeEntry(getBusinessObject(element), element, extensionElements),
        );
      } else {
        commands.push(
