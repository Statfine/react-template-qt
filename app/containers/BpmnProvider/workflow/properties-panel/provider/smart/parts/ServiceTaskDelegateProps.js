    ...
    entryFactory.link({
      ...
      showLink(element, node) {
        ...
        if (isConnector(element)) {
          const connectorId = InputOutputHelper.getConnector(element).get(
            'connectorId',
          );
          if (connectorId) {
            link.textContent = translate('Configure Connector');
          } else {
            link.innerHTML = `<span class="bpp-icon-warning"></span> ${escapeHTML(
              translate('Must configure Connector'),
            )}`;
