...
function getContainedSmartes(element) {
...
function getSmartesForActivityRef(element) {
  var bo = getBusinessObject(element);
  var parent = bo.$parent;

  var smartesForActivityRef = getSmartesForCompensation(parent);

  // if throwing compensation event is in an event sub process:
  // get also all smartes outside of the event sub process
  if (is(parent, 'bpmn:SubProcess') && parent.triggeredByEvent) {
    parent = parent.$parent;
    if (parent) {
      smartesForActivityRef = smartesForActivityRef.concat(getSmartesForCompensation(parent));
    }

  }

  return smartesForActivityRef;
}

function createActivityRefOptions(element) {
  var options = [ { value: '' } ];

  var smartes = getSmartesForActivityRef(element);
