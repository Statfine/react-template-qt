...
export default class Gaode extends PureComponent {
  componentDidMount() {
    ...
    const map = new Map({
      ...
      layers: [navlayer],
      view: new View({
        center: olProj.transform([113.934514, 22.540515], 'EPSG:4326', 'EPSG:3857'), // 数据存储在EPSG：4326([106.51, 29.55])中并显示在 EPSG：3857([12958752, 4848452])中
        zoom: 13,
        minZoom: 3
      })
    });
  ...
  showClickPopup = (map) => {
    ...
    map.on('click',function(e){
      ...
      console.log('pixel', pixel, coordinate, olProj.transform(coordinate, 'EPSG:3857', 'EPSG:4326'));
  ...
  addPoint = (map) => {
    ...
    const feature = new Feature({
      // 坐标转换 默认3857
      geometry: new Point(olProj.transform([113.91069823740762, 22.518223359444804], 'EPSG:4326', 'EPSG:3857'))
    });
  ...
  toLine = (map) => {
    const coordinates = [[113.912949, 22.537602], [113.934514, 22.540515], [113.91069823740762, 22.518223359444804]];
  ...
  render() {
    return (
      <>
        <div style={{ width: '800px', height: '400px' }} id="map"><div id="popup1">popup</div></div>
